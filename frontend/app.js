...
function home(){
  const last=findLesson(read(STORAGE.lastLesson,state.lessonId)) || allLessons()[0]; const lastQuarter=findQuarter(read(STORAGE.lastQuarter,state.quarterId)) || findQuarter('h3q2');
...
function profile(){const last=findLesson(read(STORAGE.lastLesson,state.lessonId))||allLessons()[0],lastQuarter=findQuarter(read(STORAGE.lastQuarter,state.quarterId))||findQuarter('h3q2'),activity=progressCache.activity;return `<div class="shell profile"><div class="profile-head"><div class="avatar">${esc(state.session.name.slice(0,2))}</div><div><h1 class="headline" style="margin:0">${esc(state.session.name)}</h1><p class="sub">طالب في مدرسة القرآن</p></div></div><section class="grid activity-grid"><article class="card"><span class="eyebrow">آخر درس</span><h3>${esc(last.title)}</h3><button class="text-link" onclick="openLesson('${last.id}')">استئناف الدرس ←</button></article><article class="card"><span class="eyebrow">آخر ربع</span><h3>${esc(lastQuarter.hizb.title)} · ${esc(lastQuarter.name)}</h3><button class="text-link" onclick="openQuarter('${lastQuarter.id}')">فتح الربع ←</button></article><article class="card"><span class="eyebrow">تقدّم المنهج</span><h3>${allLessons().filter(lesson=>lessonStatus(lesson.id)==='completed').length} من ${allLessons().length} درس</h3>${progressBar(curriculumPercent())}</article></section><section class="card" style="margin-top:17px"><div class="section-title"><h3>نشاط الاستماع الأخير</h3></div>${activity.length?activity.map(item=>`<div class="activity-row"><b>${esc(item.title)}</b><span>${esc(item.kind)} · ${fmt(item.date)}</span></div>`).join(''):empty('ابدأ الاستماع إلى أي تسجيل ليظهر نشاطك هنا.')}</section></div>`}
...
async function uploadResource(event){event.preventDefault();if(state.session?.role!=='admin')return;const area=$('#upload-area').value,targetId=$('#upload-target').value,title=$('#upload-title').value.trim(),type=$('#upload-type').value,file=$('#upload-file').files[0];if(!title||!file)return;const form=new FormData();form.append('area',area);form.append('targetId',targetId);form.append('title',title);form.append('type',type);form.append('file',file);try{await apiFetch('/upload',{method:'POST',body:form});await loadAll();event.target.reset();alert('تم رفع المورد وربطه بالمحتوى المختار.');render()}catch(err){alert(err.message)}}
