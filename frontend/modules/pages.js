...
function nav() {
  return `<header class="topbar"><div class="shell"><button class="brand" onclick="go('home')"><img src="logo.png" alt="مدرسة القرآن" class="brand-logo"></button><nav class="nav" aria-label="التنقل الرئيسي">${[
    ["home", "الرئيسية"],
    ["curriculum", "المنهج"],
    ["quran", "القرآن"],
    ["khutbahs", "الخطب"],
    ...
    .join(
      "",
    )}</nav><div class="actions"><button class="theme-toggle" onclick="toggleTheme()" aria-label="تبديل المظهر">${currentTheme() === "dark" ? '<span class="icon">light_mode</span>' : '<span class="icon">dark_mode</span>'}</button><button class="icon-btn" onclick="go('search')" aria-label="بحث"><span class="icon">search</span></button>${isAdmin(state.session) ? `<button class="icon-btn" onclick="go('admin')" aria-label="لوحة الإدارة"><span class="icon">settings</span></button>` : ""}<button class="avatar" onclick="go('profile')">${esc((state.session.name || "?").slice(0, 2))}</button><button class="text-link" onclick="logout()">خروج</button></div></div></header>`;
}
function mobileNav() {
...
function miniPlayer() {
  ...
  const label = gs.isPlaying ? '<span class="icon">pause</span>' : '<span class="icon">play_arrow</span>';
  const dur = gs.duration > 0 ? formatTime(gs.duration) : "";
  const hidden = isMiniHidden();
  const expanded = isMiniExpanded();
  const expandIcon = expanded ? '<span class="icon">close_fullscreen</span>' : '<span class="icon">open_in_full</span>';
  const shouldAnimate = isMiniAnimating();
  const seekPct = gs.duration > 0 ? Math.round((gs.currentTime / gs.duration) * 100) : 0;
  return `<div class="mini-player${expanded ? " expanded" : ""}${shouldAnimate ? " mp-animate-in" : ""}" id="mini-player" data-hidden="${hidden ? "1" : "0"}" style="${hidden ? "display:none" : ""}"><div class="mp-inner"><button class="mp-close" onclick="miniClose()" aria-label="إغلاق"><span class="icon">close</span></button><div class="mp-expanded-art" aria-hidden="true"><div class="mp-artwork"><span class="icon">mic</span></div></div><div class="mp-top"><div class="mp-title" id="mp-title">${esc(gs.recordingTitle)}</div></div><div class="mp-seek-row"><span id="mp-elapsed">${formatTime(gs.currentTime)}</span><div class="seek-track" role="slider" aria-label="موقع التسجيل" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${seekPct}" tabindex="0" onpointerdown="onSeekTrackPointerDown(event)"><div class="seek-track-fill" style="width:${seekPct}%"></div><div class="seek-track-thumb" style="right:${seekPct}%"></div></div><span id="mp-dur">${dur}</span></div><div class="mp-transport"><button class="mp-ctrl mp-prev" onclick="miniPrevLesson()" aria-label="الدرس السابق"><span class="icon">skip_next</span></button><button class="mp-play" onclick="miniPlayPause()" aria-label="تشغيل/إيقاف">${label}</button><button class="mp-ctrl mp-next" onclick="miniNextLesson()" aria-label="الدرس التالي"><span class="icon">skip_previous</span></button></div><div class="mp-utils"><button class="mp-speed" onclick="miniCycleSpeed(this)" aria-label="تغيير السرعة">${gs.playbackRate}×</button><button class="mp-ctrl mp-expand" onclick="miniExpand()" aria-label="توسيع"><span class="mp-expand-icon">${expandIcon}</span></button></div></div></div>${hidden ? `<button class="mini-player-fab" id="mini-player-fab" onclick="miniReopen()" aria-label="إعادة فتح المشغل"><span class="icon">play_arrow</span></button>` : `<button class="mini-player-fab" id="mini-player-fab" style="display:none" onclick="miniReopen()" aria-label="إعادة فتح المشغل"><span class="icon">play_arrow</span></button>`}`;
}

function socialButtons() {
  const cfg = state.authConfig || {};
  const googleEnabled = cfg.google?.enabled;
  const facebookEnabled = cfg.facebook?.enabled;
  if (!googleEnabled && !facebookEnabled) return "";
  const base = API_BASE.replace(/\/api\/?$/, "");
  const googleUrl = `${base}/api/auth/google`;
  const facebookUrl = `${base}/api/auth/facebook`;
  const googleBtn = googleEnabled
    ? `<a href="${googleUrl}" class="btn-social btn-google"><svg viewBox="0 0 24 24" width="18" height="18"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg> متابعة باستخدام Google</a>`
    : "";
  const facebookBtn = facebookEnabled
    ? `<a href="${facebookUrl}" class="btn-social btn-facebook"><svg viewBox="0 0 24 24" width="18" height="18"><path fill="#1877F2" d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg> متابعة باستخدام Facebook</a>`
    : "";
...
function resetPassword() {
  return `<main class="login"><button class="theme-toggle login-theme-toggle" onclick="toggleTheme()" aria-label="تبديل المظهر">${currentTheme() === "dark" ? '<span class="icon">light_mode</span>' : '<span class="icon">dark_mode</span>'}</button><section class="card login-card"><div class="brand"><img src="logo.png" alt="مدرسة القرآن" class="brand-logo"></div><h1 class="headline">تعيين كلمة مرور جديدة</h1><p class="sub">أدخل كلمة المرور الجديدة لحسابك.</p><form onsubmit="updatePassword(event)"><div class="field"><label for="new-password">كلمة المرور الجديدة</label><div class="password-wrap"><input id="new-password" required type="password" minlength="8" placeholder="8 أحرف على الأقل"><button class="password-toggle" type="button" onclick="togglePasswordVisibility(this)" aria-label="إظهار كلمة المرور" tabindex="-1"><span class="icon icon-on">visibility</span><span class="icon icon-off" style="display:none">visibility_off</span></button></div></div><div class="field"><label for="confirm-password">تأكيد كلمة المرور</label><div class="password-wrap"><input id="confirm-password" required type="password" minlength="8" placeholder="8 أحرف على الأقل"><button class="password-toggle" type="button" onclick="togglePasswordVisibility(this)" aria-label="إظهار كلمة المرور" tabindex="-1"><span class="icon icon-on">visibility</span><span class="icon icon-off" style="display:none">visibility_off</span></button></div></div><button class="primary" type="submit">تحديث كلمة المرور</button></form></section></main>`;
}
function home() {
...
function tajweedResources() {
  ...
  return `<section class="card tajweed-resources"><div class="section-title"><h3>موارد التجويد</h3></div><div class="resources tajweed-resources-grid">${resources.length ? resources.map((r) => resourceLink(r, `رفع ${fmt(r.uploadedAt)}`)).join("") : empty("لا توجد موارد تجويد عامة بعد.")}</div></section>`;
}
function curriculum() {
...
function lesson() {
  ...
  const boardHero = boardImage
    ...
    : "";
  return `<div class="shell lesson-page">${crumbs(["المنهج", lessonObj.chapter.name, lessonObj.title])}${boardHero}<h1>${esc(lessonObj.title)}</h1><p class="sub">${esc(lessonObj.description)}</p><section class="card objectives"><div class="section-title"><h3>أهداف الدرس</h3>${
    lessonStatus(lessonObj.id) === "completed"
      ? `<button class="open" disabled aria-disabled="true"><span class="icon">check_circle</span> مكتمل</button>`
...
function quarter() {
  ...
  }${quarterObj.notes ? `<p class="note"><b>ملاحظة الشيخ:</b> ${esc(quarterObj.notes)}</p>` : ""}${quarterObj.resources.length ? `<div class="resources">${quarterObj.resources.map((resource) => resourceLink(resource)).join("")}</div>` : ""}<button class="primary" onclick="toggleQuarter('${esc(quarterObj.id)}')">${done ? '<span class="icon">check_circle</span> تمت المراجعة' : "تحديد كمكتمل"}</button></section></div>`;
}
function khutbahs() {
